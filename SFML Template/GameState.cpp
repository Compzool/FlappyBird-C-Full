...
namespace Flappy
{
	...
	void GameState::Init()
	{
		if (!_hitSoundBuffer.loadFromFile(HIT_SOUND_FILEPATH)) /*error losding sound effect*/
		{
			std::cout << "Error Loading Hit Sound Effect" << std::endl;
		}

		if (!_wingSoundBuffer.loadFromFile(WING_SOUND_FILEPATH))	/*error losding sound effect*/
		{
			std::cout << "Error Loading Wing Sound Effect" << std::endl;
		}

		if (!_pointSoundBuffer.loadFromFile(POINT_SOUND_FILEPATH))	/*error losding sound effect*/
		{
			std::cout << "Error Loading Point Sound Effect" << std::endl;
		}

		_hitSound.setBuffer(_hitSoundBuffer); //hit sound effect
		_wingSound.setBuffer(_wingSoundBuffer);//wing sound effect
		_pointSound.setBuffer(_pointSoundBuffer); //point sound effect
